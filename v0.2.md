...
好，我把 **v0.2 的闭环最后一块**一次性补齐：

* ✅ `training/train.py`：训练 + MLflow 记录 + **注册到 Model Registry** + 自动打 tag（数据/特征 delta version、git commit）
* ✅ 把 `bronze/features` 从 Makefile 的 `python -c` 拆成**标准脚本**：`pipelines/bronze/run.py`、`pipelines/features/build.py`（都有 MLflow run + quality + lineage）
* ✅ 更新 `Makefile`：从“一坨命令”变成“平台式命令”

下面所有文件内容你可以直接复制进 repo。

---

# 1) 更新 Makefile（脚本化 + 可读）
...
.PHONY: help bootstrap bronze features train batch-score

help:
	@echo "SoloLakehouse v0.2 commands:"
	@echo "  make bootstrap     - init MinIO buckets"
	@echo "  make bronze        - CSV -> Bronze Delta"
	@echo "  make features      - Bronze -> Offline Feature Store (Delta)"
	@echo "  make train         - Train + log to MLflow + register model"
	@echo "  make batch-score   - Batch inference -> Predictions Delta"
	@echo ""
	@echo "Run infra first:"
	@echo "  docker compose -f infra/docker-compose.yml --env-file infra/.env up -d"
	@echo "Then:"
	@echo "  export $$(cat infra/.env | xargs)"
	@echo "  make bootstrap bronze features train batch-score"

bootstrap:
	python -m pipelines.bootstrap.bootstrap

bronze:
	python -m pipelines.bronze.run

features:
	python -m pipelines.features.build
...
# 2) 新增：`pipelines/bronze/run.py`

> 功能：读 `data/sample/data.csv` → 写入 `bronze_raw` Delta → quality gate → lineage log → MLflow run
...
def main():
    ...
    spark = get_spark("sololakehouse-bronze")
    catalog = load_catalog("tables.yaml")

    src_csv = os.environ.get("BRONZE_SOURCE_CSV", "data/sample/data.csv")
    out_path = table_path(catalog, "bronze_raw")

    with mlflow.start_run(run_name="bronze_ingest") as run:
        mlflow.set_tag("pipeline.stage", "bronze")
        mlflow.set_tag("source.csv", src_csv)
        mlflow.set_tag("delta.output.path", out_path)

        df = spark.read.option("header", "true").csv(src_csv, inferSchema=True)

        # Minimal sanity: ensure required columns exist
        required = {"id", "x1", "x2", "label"}
        missing = required.difference(set(df.columns))
        if missing:
            raise ValueError(f"Missing columns in source CSV: {sorted(missing)}")

        df.write.format("delta").mode("overwrite").save(out_path)

        # Quality gate + lineage
        run_quality_gate(spark, catalog, "bronze_raw")
        log_lineage(spark, catalog, table_name="bronze_raw", action="write_overwrite")

        mlflow.log_metric("rows", df.count())
        print("[DONE] bronze ->", out_path)
...
# 3) 新增：`pipelines/features/build.py`

> 功能：读 `bronze_raw` → 构建 `features_v1`（离线特征表）→ quality + lineage + MLflow run
...
def main():
    ...
    spark = get_spark("sololakehouse-features")
    catalog = load_catalog("tables.yaml")

    bronze_path = table_path(catalog, "bronze_raw")
    feat_path = table_path(catalog, "features_v1")

    with mlflow.start_run(run_name="build_features_v1") as run:
        mlflow.set_tag("pipeline.stage", "features")
        mlflow.set_tag("delta.input.path", bronze_path)
        mlflow.set_tag("delta.output.path", feat_path)

        bronze = spark.read.format("delta").load(bronze_path)

        # Minimal feature set (v0.2 demo)
        features = (
            bronze
            .withColumn("feature_ts", F.current_timestamp())
            .withColumn("x1_scaled", F.col("x1") * F.lit(1.0))
        ...
        run_quality_gate(spark, catalog, "features_v1")
        log_lineage(spark, catalog, table_name="features_v1", action="write_overwrite")

        mlflow.log_metric("rows", features.count())
        mlflow.log_metric("n_features", 4)  # x1,x2,x1_scaled,x2_scaled
        print("[DONE] features ->", feat_path)
        print("[MLFLOW] run_id:", run.info.run_id)
...
# 4) 新增：`pipelines/training/train.py`

> 功能：
>
> * 读 `features_v1`
> * 训练（sklearn logistic regression）
> * MLflow 记录 metrics + model artifact
> * **注册模型到 MLflow Model Registry**（`MODEL_NAME`）
> * 自动尝试将新版本设置为 `Staging`（如果 registry 权限允许）
> * lineage log（把 “训练用了哪些 Delta 表版本” 作为 tags + lineage table）
...
def main():
    ...
    # If you use MLflow Registry, set this explicitly when needed:
    # os.environ["MLFLOW_REGISTRY_URI"] = tracking_uri

    model_name = os.environ.get("MODEL_NAME", "sololakehouse-demo-model")
    experiment_name = os.environ.get("MLFLOW_EXPERIMENT_NAME", "sololakehouse")

    mlflow.set_experiment(experiment_name)

    spark = get_spark("sololakehouse-train")
    catalog = load_catalog("tables.yaml")

    feat_path = table_path(catalog, "features_v1")
    trainset_path = table_path(catalog, "training_set_v1")  # optional in v0.2

    # Load features (small demo: convert to pandas)
    sdf = spark.read.format("delta").load(feat_path)

    # In v0.2 we treat label_ts as "now" for training_set_v1 (placeholder for PIT join in v0.3)
    # Write a training_set table so the lifecycle looks like a real lakehouse.
    from pyspark.sql import functions as F
    train_sdf = sdf.withColumn("label_ts", F.current_timestamp())
    train_sdf.write.format("delta").mode("overwrite").save(trainset_path)

    # Convert to pandas for sklearn
    pdf = train_sdf.select("x1", "x2", "label").toPandas()
    X = pdf[["x1", "x2"]]
    y = pdf["label"]

    with mlflow.start_run(run_name="train_logreg_v0_2") as run:
        mlflow.set_tag("pipeline.stage", "training")
        mlflow.set_tag("model.name", model_name)
        mlflow.set_tag("delta.features.path", feat_path)
        mlflow.set_tag("delta.training_set.path", trainset_path)

        # Train
        model = LogisticRegression(max_iter=200)
        model.fit(X, y)

        # Metrics (on train for v0.2 demo; replace with split/CV in v0.3)
        ...
        mlflow.log_metric("train_f1_weighted", float(f1))

        # Log & register model
        # Note: this creates a model version under `model_name`
        ...
        # Lineage: log the key tables used/produced
        log_lineage(
            spark,
            catalog,
            table_name="training_set_v1",
            action="write_overwrite",
            extra={"model_name": model_name, "mlflow_run_id": run.info.run_id},
        )

        # Try stage transition (optional; might fail depending on permissions / MLflow setup)
        try:
            ...
            client.transition_model_version_stage(
                name=model_name,
                version=mv.version,
                stage=os.environ.get("MODEL_DEFAULT_STAGE", "Staging"),
                archive_existing_versions=False,
            )
            mlflow.set_tag("model.stage_set", "true")
            mlflow.set_tag("model.version", str(mv.version))
        except Exception as e:
            mlflow.set_tag("model.stage_set", "false")
            mlflow.set_tag("model.stage_error", str(e)[:200])

        print("[DONE] training complete")
        print("[MLFLOW] run_id:", run.info.run_id)
        print("[MODEL] registered_name:", model_name)
        print("[MODEL] model_uri:", result.model_uri)
...
# 5) 小更新：建议给 `pipelines/inference/batch_score.py` 加一个默认 experiment（可选但更整洁）

你现在的 batch_score 可以用，但为了 UI 里更统一，建议在 `main()` 最前面加：

```python
mlflow.set_experiment(os.environ.get("MLFLOW_EXPERIMENT_NAME", "sololakehouse"))
```
...
# 4) bronze -> features -> train
make bronze
make features

# 5) train (register model)
export MODEL_NAME=sololakehouse-demo-model
export MLFLOW_EXPERIMENT_NAME=sololakehouse
make train

# 6) batch inference (default uses models:/MODEL_NAME/Staging)
export MODEL_STAGE=Staging
make batch-score
```
...
* `bronze_ingest` run（写 bronze + 质量/lineage tags）
* `build_features_v1` run（写 features + 质量/lineage tags）
* `train_logreg_v0_2` run（训练、注册模型、尝试 stage）
* `batch-score` run（推理写 predictions + lineage）

---

## 你现在已经是“平台化 repo”了（为什么强）

你已经具备了一个 Lakehouse 系统最核心的“可信闭环”：

* **Catalog**：tables.yaml
* **Quality Gate**：每次写表都检查
* **Lineage**：表版本 + git commit + mlflow run 绑定
* **Registry**：模型版本化 + stage（尽力自动化）
* **Inference**：预测结果回写 Delta（形成业务输出）

---

如果你想继续把它变成“更像 Databricks 的灵魂部分”，下一步就是 **v0.3：PIT Join（Point-in-Time）**。
我可以直接给你：

* `pipelines/pit_join/build_training_set.py`（label_ts ↔ feature_ts 的正确 join）
* 一个最小 `labels` 表 + `features` 表的时间对齐示例
* 以及更真实的 `train/val/test split` + metrics logging

你回我一句：**“继续 v0.3：PIT Join”** 我就接着往下生成。
