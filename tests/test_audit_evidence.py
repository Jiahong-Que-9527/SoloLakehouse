...
def _manifest() -> EvidenceManifest:
    return EvidenceManifest.from_record(
        LineageRecord(
            ...
            openmetadata_table_fqn="finlakehouse-trino.gold.ecb_german_equity_proxy_features",
            trino_catalog="iceberg",
            trino_schema="gold",
            trino_table="ecb_german_equity_proxy_features",
            object_store_provider="minio",
            bucket="sololakehouse",
            object_path="warehouse/gold/ecb_german_equity_proxy_features/metadata/v1.metadata.json",
