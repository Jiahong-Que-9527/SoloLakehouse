...
- 默认执行入口切换：
  ...
  - 兼容旧脚本（等价）：`make pipeline-v1`、`make pipeline-legacy` 或 `make pipeline PIPELINE_MODE=v1`
...
### Superset（可选 BI / SQL UI）

```bash
make up-superset      # 启动 Superset（自动创建 superset_metadata DB）
make verify-superset  # 验证服务健康
# UI：http://localhost:8088（默认 admin / admin）
```

Superset 也以独立 compose profile 运行，不影响默认 `make up`。启动后会自动预置两个 Trino 连接：
...
- v2.5 Superset 运行入口：`docker/docker-compose.superset.yml`
