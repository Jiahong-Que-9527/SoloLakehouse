...
- 用 `trino_iceberg_gold` 浏览 `iceberg.gold.ecb_dax_features_iceberg`
- 用 `trino_hive_default` 浏览 `hive.gold.ecb_dax_features`
- 在 SQL Lab 或 Chart Builder 中快速做 Gold 特征表可视化
...
make up-openmetadata # 可选：启动 OpenMetadata
make verify-openmetadata # 可选：验证 OpenMetadata
make up-superset    # 可选：启动 Superset
make verify-superset # 可选：验证 Superset
...
以下能力在 v3 **生产化基线**中被显式标为非默认目标，避免过早引入复杂度（详见 [roadmap.md](roadmap.md)）：

- Kafka / Flink（流处理）
- 强制迁移到 OpenMetadata / DataHub（统一数据目录）
- 强制全量替换为 Delta Lake / Iceberg（表格式一刀切）
- 在线模型服务（Online Serving）
- Keycloak 级别的终端用户身份系统
- Superset / FastAPI

**v2.5 参考扩展（本仓库已落地）**：Gold 层已用 **Iceberg**（Trino `iceberg` catalog）；可选用 `make up-openmetadata` 启动 **OpenMetadata** 做元数据发现，也可用 `make up-superset` 启动 **Superset** 做 SQL / dashboard 展示。这与「v3 不强制企业目录迁移」并不矛盾：v2.5 是教学/参考增强，v3 仍不要求你必须强制接入这些可选 UI。
