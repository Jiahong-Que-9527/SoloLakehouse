...
**v2.5 参考扩展（本仓库已落地）**：Gold 层已用 **Iceberg**（Trino `iceberg` catalog）；可选用 `make up-openmetadata` 启动 **OpenMetadata** 做元数据发现。这与「v3 不强制企业目录迁移」并不矛盾：v2.5 是教学/参考增强，v3 仍不要求你必须上 OM。
